...
async def magic_wand(project_id: str, payload: MagicWandModel):
    ...
    target_color = CATEGORY_COLORS.get(payload.category, (0, 0, 255))
    mask_b64, pixel_area, pixel_length = process_magic_wand(
        image_data, payload.x, payload.y, payload.tolerance,
        category=payload.category, color=target_color
    )
    real_area = None
    real_length = 0
    if project.get("scale_factor"):
        ...
        real_length = pixel_length / sf
    return {
        "mask_image": mask_b64, "pixel_area": pixel_area, "real_area": real_area,
        "real_length": real_length, "unit": project.get("unit", "px"), "category": payload.category
    }
...
async def draw_opening(project_id: str, payload: LineToolModel):
    ...
    if "masks" in project:
        for m in project["masks"]:
            if m.get("category") == "Walls":
                wall_masks_bytes.append(base64.b64decode(m["src"].replace("data:image/png;base64,", "")))
    mask_b64, pixel_area, pixel_length = process_linear_opening(
        image_data, wall_masks_bytes, payload.p1, payload.p2, payload.category, color=target_color
    )
    real_area = real_length = 0
    if project.get("scale_factor"):
        sf = project["scale_factor"]
        real_area = pixel_area / (sf ** 2)
        real_length = pixel_length / sf
    return {"mask_image": mask_b64, "real_area": real_area, "real_length": real_length, "category": payload.category}
